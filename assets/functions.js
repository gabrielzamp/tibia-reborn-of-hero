...
const stage = {
  ...
  update() {
    ...
    this.fighter1El.querySelector(".bar").style.width - `${f1Pct.maxLife}$`;
    //Fighter 2
    ...
    this.fighter2El.querySelector(".bar").style.width - `${f2Pct.maxLife}$`;
  },
  doAttack(attacking, attacked) {
    ...
    if (actualAttack > actualDefense) {
      ...
      log.addMessage(
        `- ${attacking.name} atacou e causou ${actualAttack.toFixed(
          2
        )} de dano...`
      );
    } else {
      log.addMessage(
        `- ${attacking.name} atacou e ${attacked.name} conseguiu se defender...`
      );
    }
